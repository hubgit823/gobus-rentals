  ...
  const loginMut = useMutation({
    ...
    onSuccess: (data) => {
      if (data.user.role !== "customer") {
        toast.error("Use vendor or admin login for this account.");
        return;
      }
              ...
              <TabsContent value="email">
                ...
                >
                  ...
                  <div className="flex justify-end">
                    <a href="mailto:support@luxurybusrental.com?subject=Password%20reset" className="text-xs text-primary hover:underline">
                      Forgot password?
                    </a>
                  </div>
                  <Button className="w-full" size="lg" type="submit" disabled={loginMut.isPending}>
                    {loginMut.isPending ? "Signing in…" : "Login"} <ArrowRight className="w-4 h-4" />
                  </Button>
                </form>
              </TabsContent>
